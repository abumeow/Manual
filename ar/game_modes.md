...
لعرض حالة التقدم في السيطرة، يجب أن تكون موجودًا فعليًا داخل نطاق نقطة التحكم كما هو موضح على الخريطة، ثم تضغط على زر عرض قائمة اختيار الكِت/الفرقة (Caps Lock) أو زر الخريطة (M).
...
## _القيادة والتحكم (Command & Control – CNC)_ {#command-and-control}
...
## _التعاون (Co-Operative – COOP)_ {#co-operative}
...
## _التمرد (Insurgency – INS)_ {#insurgency}

يُعد هذا النمط فريدًا مقارنة ببقية أنماط اللعب في PR. هدف قوات التحالف (BluFor) هو العثور على مخابئ الأسلحة (Weapon Caches) التابعة لفصيل المتمردين (RedFor أو OpFor) وتدميرها وهي مخفية في أنحاء الخريطة. يختلف عدد مخابئ الأسلحة المطلوب تدميرها حسب الخريطة والنسخة، وغالبًا يكون هناك 6 مخابئ الأسلحة ويجب تدمير 5 منها للفوز. يفوز المتمردون إذا تمكنوا من حماية ما لا يقل عن  حتى تنفد تذاكر قوات التحالف. بالنسبة لقوات التحالف، تطبق قواعد خسارة التذاكر الخاصة بنمط AAS، بينما لا يمتلك المتمردون تذاكر ويمكنهم إعادة الظهور بلا حدود.
{% figure "../assets/weaponcache.png" %}
مخابئ الأسلحة (Weapon Cache){% endfigure %}

يجب على قوات التحالف جمع المعلومات الاستخباراتية عبر قتل مقاتلي المتمردين أو أسر المدنيين [ألقِ نظرة على مدني](the_civilian.md).لأسر مدني، يجب أن يكون لاعب التحالف على بعد متر واحد من المدني، ويجب استخدام أداة تقييد (أصفاد بلاستيكية)، والتي يمكن الوصول إليها عبر اختيار الرقم 1 في لوحة المفاتيح، أو بديلًا يمكن استخدام بندقية (Shotgun) لمحاكاة التعطيل برصاص غير قاتل.

يفوز التحالف إذا تمكن من تدمير عدد معين من مخابئ الأسلحة قبل نفاد الوقت أو التذاكر، وإلا يفوز المتمردون. يمكن لمهندسي المتمردين إصلاح مخبأ الأسلحة التالفة. يتم عرض عدد مخابئ الأسلحة عند بداية الجولة وعلى الخريطة، كما يمكن للمتمردين معرفة العدد عبر قائمة الكِت/الفرقة (Caps Lock).

لكشف الموقع التقريبي لمخبأ الأسلحة أثناء نمط Insurgency، يجب جمع معلومات استخباراتية. يحتاج فريق التحالف إلى 50 نقطة معلومات ليتم الكشف عن المخبأ بعد 5 دقائق. يتم كسب أو فقدان النقاط كالتالي:
...
لا يمكن للدبابات أو الطائرات أو المروحيات كسب نقاط المعلومات الأسنخبارتية IP عبر هذه الطرق.
يظهر الموقع التقريبي لأول مخبأ بعد حوالي 5 دقائق من بداية الجولة. يمكن أن يوجد حتى مخبأين نشطين في نفس الوقت. يتم كشف مواقع جديدة عند الوصول إلى 50 نقطة معلومات. إذا كان عدد اللاعبين أقل من 32، يتم تقليل متطلبات النقاط إلى النصف. يمكن أن تتراوح النقاط بين -50 و +50 كحد أقصى.

علامات الخريطة في نمط Insurgency:

* ![](../assets/cache.png) **مخبأ أسلحة معروف لقوات التحالف:** يظهر على خريطة التحالف عند توفر معلومات كافية، وقد يكون موقعه التقريبي منحرفًا حتى 75 مترًا.
* ![](../assets/unknown%20weapon%20cache.png) **مخبأ أسلحة معروف للمتمردين**: يظهر هذا الرمز على خريطة قوات المتمردين، ويشير إلى أن قوات التحالف قد جمعت معلومات استخباراتية كافية لتحديد موقع المخبأ. بمجرد ظهور هذا الرمز، يتم إنشاء (Spawn) مخبأ الأسلحة فعليًا في ساحة المعركة، ليصبح قابلًا للاستخدام من قِبل المتمردين، كما يصبح هدفًا يمكن لقوات التحالف تدميره. ومع ذلك، توجد فترة سماح مدتها 5 دقائق بين ظهور المخبأ في اللعبة وبين كشف موقعه لقوات التحالف.
* ![](../assets/unknown%20cache.png) **مخبأ غير معروف للمتمردين:** يمثل الموقع الذي سيظهر فيه المخبأ لاحقًا، ولا يظهر إلا للمتمردين لتنظيم الدفاعات.

يمكن تدمير مخبأ ألاسلحة باستخدام C4 (صغيرة أو كبيرة)، بينما الأسلحة الأخرى تسبب ضررًا ضعيفًا أو معدومًا.

عند تدمير مخبأ ألاسحلة، يحصل فريق التحالف على 30 تذكرة، ويتم إعلام الفريق بعدد المخابئ المتبقية.
يمكن إصلاح مخابئ الأسلحة باستخدام مفتاح الصيانة (Wrench) أو مجرفة (Shovel).


لا يتم إظهار مخبأ ألاسحلة على الخريطة إلا قبل 30 ثانية من بداية الجولة لتجنب استغلال المعلومات.


## _لعبة الأسلحة (Gungame)_ {#gungame}
